...
void encode(char key[], char input[], char output[], int length);
  ...
  while(1)
  {
		...
		switch (pid) 
    {
			...
			case 0:
      {
        ...
        encode(keyText, inputText, outputText, strlen(inputText));
...
void encode(char key[], char input[], char output[], int length) 
{
  ...
  // Convert key array to int
	...
	int integerOutput[length];
	for (int i = 0; i < length; i++) 
  {
		integerOutput[i] = (integerInput[i] + integerKey[i]) % 27;
	}

	// Convert output to char
	convertIntToChar(integerOutput, output, length);
	output[length] = '\0';
}
